...
class Example(Frame):
    ...
    def Build (self, value):
        ...
        self.canvas.create_text(((x + x + 200) / 2), ((y + y+75) / 2), text=value, font=("Arial", 20))

    def Size(self, prev_value, next_value):
        if (prev_value in self.dict_size) and (self.dict_size[prev_value] < 3):
    ...
    def initUI(self):
        ...
        btn_exit = Button(text="Create", font=("Arial", 30), activebackground="#fa0202", fg="#bd2626",
        ...
        prev_text = Label(text="Enter the previous graph", font=("Arial", 20))
        self.canvas.create_window(180, 50, window=prev_text)

        enter_prev = Entry(width=20, font=("Arial", 20))
        self.canvas.create_window(155, 100, window=enter_prev)

        next_text = Label(text="Enter the next graph", font=("Arial", 20))
        self.canvas.create_window(165, 150, window=next_text)

        enter_next = Entry(width=20, font=("Arial", 20))
        self.canvas.create_window(155, 200, window=enter_next)
